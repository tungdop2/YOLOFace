...
class YoloDetector:
    def __init__(self, weights_name='yolov7-face_state_dict.pt', config_name='yolov7-face.yaml', device='cuda:0', min_face=30, target_size=None, frontal=False):
            """
    ...
    def _postprocess(self, imgs, origimgs, pred, conf_thres, iou_thres):
        ...
        for i in range(len(origimgs)):
            ...
            det = pred[i].cpu()
            scale_coords(imgs[i].shape[1:], det[:,:4], img_shape, kpt_label=False).round()  # native-space pred
            scale_coords(imgs[i].shape[1:], det[:,6:], img_shape, kpt_label=True, step=3).round()  # native-space pred
            ...
            for j in range(det.size()[0]):
                ...
                if box[3] - box[1] < self.min_face:
                    continue
                lm = (det[j, 6:21].view(5, 3)[...,:2].reshape(-1) / gn_lks).view(-1).tolist()
    ...
    def predict(self, imgs, conf_thres = 0.5, iou_thres = 0.5, return_landmarks=False):
        '''
